...
describe("cross-platform tasksDir default (issue #25)", () => {
  ...
  it("resolves user-configured relative path (backward compat)", () => {
    ...
    assert.ok(
      config.storage.tasksDir.endsWith(path.join("data", "tasks")),
      `should end with "${path.join("data", "tasks")}" but got "${config.storage.tasksDir}"`,
    );
  });
...
describe("cross-platform auditLogPath default", () => {
  ...
  it("resolves user-configured relative path to absolute", () => {
    ...
    assert.ok(
      config.observability.auditLogPath.endsWith(path.join("data", "audit.jsonl")),
      `should end with "${path.join("data", "audit.jsonl")}" but got "${config.observability.auditLogPath}"`,
    );
  });
});
